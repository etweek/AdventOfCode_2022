...
fn main() {
    ...
    let needed_space = free_space_needed-free_space;
    println!("Have used {total_used} of {total_space} so need to free up an additional {needed_space}");
    let mut best:i32 = *total_used;
    for (_key, value) in sizes{
        println!("considering {value}");
        if value >= needed_space && value < best{
            println!("{value} is a good candidate, it's bigger than {needed_space} but less than {best}");
            best = value;
        }
    }
    println!("best directory to delete is of size {best}");
}

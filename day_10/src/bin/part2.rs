...
fn main() {
    ...
    let mut pixels:Vec<Vec<bool>> = vec![vec![false; 40]; 6];
    let mut cycle:u32 = 0;
    for (row,line) in input_lines.iter().enumerate(){
        ...
        if instruction == "noop"{
            cycle += 1;
            (registers, pixels) = on_tick(cycle, registers, pixels);
        }
...
//            println!("\t x={x}");
    ...
    display_screen(pixels);
}
fn display_screen(pixels: Vec<Vec<bool>>){
    for line in pixels {
        for pixel in line {
            if pixel{
                print!("#");
...
fn on_tick(cycle: u32, mut registers: HashMap<char,i32>, mut pixels: Vec<Vec<bool>>) -> (HashMap<char,i32>,Vec<Vec<bool>>) {
    ...
    // now for pixel rendering
    let row:usize = ((cycle-1)/40) as usize;
    let col:i32 = ((cycle-1)%40) as i32;
    let val = x >= col-1 && x <= col+1;
    println!("setting {row},{col}={val}");
    pixels[row][col as usize]=val;
    return (registers, pixels);
}
